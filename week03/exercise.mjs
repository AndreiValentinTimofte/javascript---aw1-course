...
function Question(id, text, email, userId, date) {
    ...
    this.getAnswers = () => {
        return new Promise((resolve, reject) => {
            // Le stringhe su più righe in JS si creano con i backtick (`), non con le virgolette (").
            // Ho anche corretto alcuni typo nella query (quesetion -> question, user.if -> user.id)
            const sql = `SELECT answer.*, user.email FROM
                        answer JOIN user ON answer.authorId = user.id 
            ...
            db.all(sql, params, (err, rows) => {
                if (err)
                    ...
                    const answers =  rows.map((ans) => new Answer(ans.id, ans.text, ans.email, 
                        ans.authorId, ans.date, ans.score))
                    resolve(answers);
                }
            });
        });
        ...
        return new Promise((resolve, reject) => {
            ...
            db.get(sql, params, (err, row) => {
                if (err)
                    ...
                    resolve(new Question(row.id, row.test, 
                    row.email, row.authorId, row.date));
                }
                else {
                    resolve("Nessuna domanda trovata, controllare l'ID.");
                }
            });
        });
    };
